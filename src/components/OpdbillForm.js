...
const OpdbillForm = () => {
  ...
  const isUpdateResponse = useSelector(
    (state) => state.opdReducer.isUpdateResponse
  );
  ...
  const handleSubmit = (event) => {
    ...
    if (!amount) {
    ...
    } else if (!re.test(particulars)) {
      alert("Enter only letters for particulars");
    } else if (!particulars) {
      ...
      try {
        const response = axios
          .post(
            "http://localhost:8080/upload-opd-bill",
              ...
              <div>
                <Button
                  ...
                  onClick={(e) => {
                    ...
                    if(amount!=""&&particulars!=""&&date!=""&&file!=null){
                        handleSubmit(e);
