...
import {
  PostExpensebillApiAction,
  UpdateExpensebillApiAction,
} from "../redux/action/expensebillAction";
...
const ExpensebillForm = () => {
  ...
  const isResponse = useSelector(
    (state) => state.expensebillReducer.isResponse
  );
  const isUpdateResponse = useSelector(
    (state) => state.expensebillReducer.isUpdateResponse
  );
  ...
  const handleSubmit = (event) => {
    ...
    try {
      const response = axios
        .post(
          "http://localhost:8080/upload-expense-bill",
      ...
      <div className="signup_container">
          ...
          ></input>
           <input
            ...
            className="input"
          ></input>
          <input type="file" onChange={handleFileSelect} className="input"/>
          <div>
